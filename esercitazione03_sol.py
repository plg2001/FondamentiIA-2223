...
def fitness_fn(individual):
    ...
    """
    nonattacking = 0
    for (j1, i1), (j2, i2) in itertools.product(enumerate(individual), enumerate(individual)):
        if j1 == j2:
            continue
        if i1 == i2:
            continue
        if abs(i1 - i2) == abs(j1 - j2):
            continue
        nonattacking += 1
    assert nonattacking % 2 == 0
    return int(nonattacking / 2)
...
def mutation(individual, rate):
    ...
    mutated[i] = random.randrange(len(individual))
    return mutated
...
def success_rate(**parameters):
    ...
    """
    n_experiments = 25
    successes = [run_ga_n_queens(**parameters) for _ in range(n_experiments)]
    r = sum([1 if success else 0 for success in successes]) / n_experiments
    return r
