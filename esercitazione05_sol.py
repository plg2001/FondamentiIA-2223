...
def value_iteration(
    ...
    for i in range(iterations):
        ...
        for s in product(range(width), range(height)):
            # If the state is an end state or we reached the max number
            # of steps, we can't perform any action.
            if s == end_state or i >= max_steps:
                continue
            # Compute:
            # - V*(s) = max_a { Q*(s, a) }
            # - pi*(s) = argmax_a { Q*(s, a) }
            # Q*(s, a) is computed only once for both V* and pi*.
            max_value = None
            a_best = None
            for a in range(num_actions):
                s1_probs = transition_probabilities[s, a]
                # Compute Q*(s, a) = sum{ p(s1|s,a) * (r(s1) + gamma*V*(s1)) }
                v = 0.0
                for s1 in product(range(width), range(height)):
                    v += s1_probs[s1] * (rewards[s1] + gamma*old_values[s1])
                if max_value is None or v > max_value:
                    max_value = v
                    a_best = a
            assert max_value is not None and a_best is not None
            values[s] = max_value
            best_actions[s] = a_best
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            if random.random() <= eps:
                ...
                # compute pi(s) = argmax_a{ Q*(s, a) }
                action = None
                qa_max = None
                for a in range(num_actions):
                    qa = q_table[state, a]
                    if action is None or qa > qa_max:
                        action = a
                        qa_max = qa
            # perform the action in the environment
            next_state, reward, done = env.step(action)
            # compute Q*(s, a) = (1-alpha)*Q*(s, a) + alpha*(r + gamma*max_a'{Q*(s',a')})
            q_old = q_table[state, action]
            q_max_next = max(q_table[next_state, a] for a in range(num_actions))
            q_table[state, action] = (1-alpha)*q_old + alpha*(reward + gamma*(1-done)*q_max_next)
            # update first received reward
            if not received_first_reward and reward > 0:
                received_first_reward = True
                print(f'Received first reward at episode {ep}.')
            # update state (very important, but very easy to forget!)
            state = next_state
            steps += 1
        ret_sum = 0.
        for _ in range(10):
            ret = eval_ql_policy(q_table)
            ret_sum += ret
        evals.append(ret_sum / 10)
        # if received first reward, decay epsilon afer each episode
